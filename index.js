...
<head>
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js">// 🌍 Dünya görselini daha gerçekçi yap ve ülke marker'ı ekle
...
window.onload = () => {
  ...
  fetch("http://localhost:3000/api/population")
    ...
    .then(data => {
      ...
      output.innerText = "🌍 The 5 most populous countries:\n\n" + top5.map(c =>
        `${c.countryName} (${c.year}): ${parseInt(c.value).toLocaleString()}`
      ).join("\n");
    })
    .catch(err => {
      const output = document.getElementById("output");
      output.innerText = "❌ Veri alınamadı: " + err.message;
    });
};
...
// Form gönderimi yapıldığında GPT çağrısı yapılsın

// Mevcut submit event listener'ı iptal edildiği için yeni listener:
